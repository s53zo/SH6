(() => {
  ...
  const COMPARE_WORKSPACE_MODULE_URL = './modules/compare/workspace-ui.js?v=6.3.16';
  const COMPARE_CONTROLLER_RUNTIME_MODULE_URL = './modules/compare/controller-runtime.js?v=6.3.16';
  const RETAINED_RUNTIME_MODULE_URL = './modules/reports/retained-runtime.js?v=6.3.16';
  const NAVIGATION_RUNTIME_MODULE_URL = './modules/ui/navigation-runtime.js?v=6.3.16';
  const STORAGE_RUNTIME_MODULE_URL = './modules/storage/runtime.js?v=6.3.16';
  const ARCHIVE_CLIENT_MODULE_URL = './modules/archive/client.js?v=6.3.16';
  const ARCHIVE_SEARCH_RUNTIME_MODULE_URL = './modules/archive/search-runtime.js?v=6.3.16';
  const LOAD_PANEL_RUNTIME_MODULE_URL = './modules/ui/load-panel-runtime.js?v=6.3.16';
  const ANALYSIS_CONTROLS_RUNTIME_MODULE_URL = './modules/ui/analysis-controls-runtime.js?v=6.3.16';
  const COACH_RUNTIME_MODULE_URL = './modules/coach/runtime.js?v=6.3.16';
  const CANVAS_ZOOM_RUNTIME_MODULE_URL = './modules/ui/canvas-zoom-runtime.js?v=6.3.16';
  const RBN_SIGNAL_EXPORT_RUNTIME_MODULE_URL = './modules/spots/signal-export-runtime.js?v=6.3.16';
  const SPOTS_COMPARE_RUNTIME_MODULE_URL = './modules/spots/compare-runtime.js?v=6.3.16';
  const SPOTS_DRILLDOWN_RUNTIME_MODULE_URL = './modules/spots/drilldown-runtime.js?v=6.3.16';
  const SPOTS_COACH_SUMMARY_RUNTIME_MODULE_URL = './modules/spots/coach-summary-runtime.js?v=6.3.16';
  const SPOTS_DIAGNOSTICS_RUNTIME_MODULE_URL = './modules/spots/diagnostics-runtime.js?v=6.3.16';
  const SPOTS_CHARTS_RUNTIME_MODULE_URL = './modules/spots/charts-runtime.js?v=6.3.16';
  const SPOTS_DATA_RUNTIME_MODULE_URL = './modules/spots/data-runtime.js?v=6.3.16';
  const SPOTS_ACTIONS_RUNTIME_MODULE_URL = './modules/spots/actions-runtime.js?v=6.3.16';
  const RBN_COMPARE_CHART_RUNTIME_MODULE_URL = './modules/spots/rbn-compare-chart-runtime.js?v=6.3.16';
  const RBN_COMPARE_VIEW_RUNTIME_MODULE_URL = './modules/spots/rbn-compare-view-runtime.js?v=6.3.16';
  const RBN_COMPARE_MODEL_RUNTIME_MODULE_URL = './modules/spots/rbn-compare-model-runtime.js?v=6.3.16';
  const RBN_COMPARE_RUNTIME_MODULE_URL = './modules/spots/rbn-compare-runtime.js?v=6.3.16';
  const INVESTIGATION_ACTIONS_RUNTIME_MODULE_URL = './modules/ui/investigation-actions-runtime.js?v=6.3.16';
  const INVESTIGATION_WORKSPACE_MODULE_URL = './modules/reports/investigation-workspace.js?v=6.3.16';
  const SESSION_CODEC_MODULE_URL = './modules/session/codec.js?v=6.3.16';
  const SESSION_PERSPECTIVES_MODULE_URL = './modules/session/perspectives.js?v=6.3.16';
  const EXPORT_RUNTIME_MODULE_URL = './modules/export/runtime.js?v=6.3.16';
  ...
  let viewContainerLogDrilldownHandler = null;
  ...
  const REQUIRED_ANALYSIS_CORE_METHODS = Object.freeze([
    ...
    'computeRuleQsoPoints',
    'computeRuleMultipliers',
    'resolveContestRuleSet',
    'computeContestScoringSummary'
  ]);

  function checkAnalysisCoreCapabilities(core = globalThis.SH6AnalysisCore) {
    return !core
      ? REQUIRED_ANALYSIS_CORE_METHODS.slice()
      : REQUIRED_ANALYSIS_CORE_METHODS.filter((name) => typeof core[name] !== 'function');
  }

  function getAnalysisCore() {
    const core = globalThis.SH6AnalysisCore;
    const missing = checkAnalysisCoreCapabilities(core);
    if (!core || missing.length) {
      const suffix = missing.length ? ` Missing: ${missing.join(', ')}.` : '';
      throw new Error(`SH6 analysis core is unavailable or outdated.${suffix}`);
    }
    return core;
  }
    ...
    const collect = (spotState, source) => {
      ...
      rows.forEach((spot) => {
        ...
        anchors.push({
          ...
          mode: spot.txMode || spot.tx_mode || spot.mode || '',
          spotter: spot.spotter || spot.spotterCall || spot.de || ''
        });
      });
    };
  ...
  function formatNumberHtmlSh6(value) {
    return escapeHtml(formatNumberSh6(value));
  }
  ...
  function normalizeScoringRuleOverride(value) {
    return getAnalysisCore().normalizeScoringRuleOverride(value);
  }

  function isWrtcScoringRuleId(value) {
    return getAnalysisCore().isWrtcScoringRuleId(value);
  }

  function getWrtcScoringRuleLabel(value) {
    return getAnalysisCore().getWrtcScoringRuleLabel(value);
  }

  function isWrtcScoringCandidate(contestMeta) {
    return getAnalysisCore().isWrtcScoringCandidate(contestMeta);
  }

  function parseClaimedScoreNumber(value) {
    return getAnalysisCore().parseClaimedScoreNumber(value);
  }

  function getArchiveFolderFromPath(path) {
    return getAnalysisCore().getArchiveFolderFromPath(path);
  }

  function applyScoringSpec(spec, sourceLabel) {
    const indexes = getAnalysisCore().buildScoringIndexes(spec);
  ...
  function computeRuleQsoPoints(rule, qsos, station, assumptions) {
    return getAnalysisCore().computeRuleQsoPoints(rule, qsos, station, assumptions, buildAnalysisResourcesPayload());
  }
  ...
  function resolveContestRuleSet(contestMeta, context = {}) {
    return getAnalysisCore().resolveContestRuleSet(contestMeta, context, buildAnalysisResourcesPayload());
  }

  function computeContestScoringSummary(qsos, contestMeta, context = {}) {
    return getAnalysisCore().computeContestScoringSummary(qsos, contestMeta, context, buildAnalysisResourcesPayload());
  }
  ...
  function buildBreakReportMetrics(derived, threshold) {
    ...
    const onAirMin = participationMin != null
      ...
      : null;
    const operatorBreakSummary = Array.isArray(derived?.operatorBreakData)
      ? derived.operatorBreakData.map((entry) => {
        const op = String(entry?.op || '').trim().toUpperCase();
        if (!op) return null;
        const opMinutes = buildMinuteMapFromList(Array.isArray(entry.minutes) ? entry.minutes : []);
        const opBreakSummary = computeBreakSummary(opMinutes, threshold);
        const entryMinTs = Number(entry?.minTs);
        const entryMaxTs = Number(entry?.maxTs);
        const opParticipationMin = Number.isFinite(entryMinTs) && Number.isFinite(entryMaxTs) && entryMaxTs >= entryMinTs
          ? Math.round((entryMaxTs - entryMinTs) / 60000)
          : null;
        const opOnAirMin = opParticipationMin != null
          ? Math.max(opParticipationMin - opBreakSummary.totalBreakMin, 0)
          : null;
        return {
          op,
          totalBreakMin: opBreakSummary.totalBreakMin,
          breakCount: opBreakSummary.breaks.length,
          participationMin: opParticipationMin,
          onAirMin: opOnAirMin
        };
      }).filter(Boolean).sort((a, b) => b.totalBreakMin - a.totalBreakMin || a.op.localeCompare(b.op))
      : [];
    return {
      breakSummary,
      participationMin,
  ...
  function parseCabrilloFreqToken(token) {
    return getAnalysisCore().parseCabrilloFreqToken(token);
  }
  ...
  function runScoringRegressionChecks() {
    const checks = [];
    const actualCapabilityMissing = checkAnalysisCoreCapabilities(globalThis.SH6AnalysisCore);
    checks.push({
      name: 'Loaded analysis core exposes all delegated scoring/parser methods',
      passed: actualCapabilityMissing.length === 0,
      details: {
        missing: actualCapabilityMissing
      }
    });

    const capabilityMissing = checkAnalysisCoreCapabilities({
      analyzeLogText() {},
      buildDerived() {}
    });
    checks.push({
      name: 'Analysis core capability guard reports delegated scoring/parser methods',
      passed: capabilityMissing.includes('computeContestScoringSummary')
        && capabilityMissing.includes('parseCabrillo')
        && capabilityMissing.includes('computeRuleQsoPoints'),
      details: {
    ...
    const previousScoringState = {
      ...
      scoringRuleMap: state.scoringRuleMap,
      scoringRuleByFolder: state.scoringRuleByFolder,
      scoringAliasMap: state.scoringAliasMap,
    ...
    checks.push({
      name: 'Main delegated scoring summary preserves WRTC 2026 result',
      passed: wrtcSummary?.computedQsoPointsTotal === 12
        && wrtcSummary?.computedMultiplierTotal === 3
  ...
  function renderLogCellById(q, columnId, options) {
    ...
    switch (columnId) {
      case 'num':
        return `<td class="log-qso c1">${formatNumberHtmlSh6(q.qsoNumber || '')}</td>`;
      case 'time': {
      ...
      case 'rstR':
        return `<td>${formatNumberHtmlSh6(q.rstRcvd || '')}</td>`;
      case 'exchSent':
        return `<td>${formatNumberHtmlSh6(q.stx || q.exchSent || '')}</td>`;
      case 'exchRcvd':
        return `<td>${formatNumberHtmlSh6(q.srx || q.exchRcvd || '')}</td>`;
      case 'op':
  ...
  function renderLogContent() {
    ...
    const rowList = filtered.slice(start, end).map((q, idx) => {
      ...
      return `
      <tr class="${idx % 2 === 0 ? 'td1' : 'td0'}">
        <td class="log-qso c1">${formatNumberHtmlSh6(q.qsoNumber || '')}</td>
        ...
        <td class="tl">${call}</td>
        <td>${formatNumberHtmlSh6(q.rstSent || '')}</td>
        <td>${formatNumberHtmlSh6(q.rstRcvd || '')}</td>
        <td>${formatNumberHtmlSh6(q.stx || q.exchSent || '')}</td>
        <td>${formatNumberHtmlSh6(q.srx || q.exchRcvd || '')}</td>
        <td>${op}</td>
  ...
  function buildWpxHourBucketMap(qsos) {
    ...
    (qsos || []).forEach((q) => {
      ...
      if (band) entry.bands.set(band, (entry.bands.get(band) || 0) + 1);
      if (continent) entry.continents.set(continent, (entry.continents.get(continent) || 0) + 1);
    });
    return map;
  }
  ...
  function buildWpxColumnOrderFromMaps(maps, mode) {
    ...
    maps.forEach((map) => {
      map.forEach((entry) => {
        if (!entry) return;
        const source = normalizedMode === WPX_COLUMN_MODE_BAND
          ? entry.bands
          : normalizedMode === WPX_COLUMN_MODE_CONTINENT
            ? entry.continents
            : entry.prefixes;
  ...
  function renderWpxByHourSheetForSlot(slot, keyOrder, columnOrder, bucketMap, mode) {
    ...
    const rows = keyOrder.map((key, idx) => {
      ...
      const cells = columnOrder.map((column) => {
        ...
        if (!count) return '<td></td>';
        const hourBucket = entry ? Math.floor(entry.sampleTs / 3600000) : null;
        if (hourBucket == null) return `<td>${formatNumberSh6(count)}</td>`;
      ...
      return `<tr class="${cls}" data-compare-row-key="hour-${escapeAttr(String(key))}"><td>${dayLabel}</td><td><b>${hourLabel}</b></td>${cells}${totalCell}<td>${formatNumberSh6(accum || '')}</td></tr>`;
    }).join('');
  ...
  function renderWpxByHourSheetCompare() {
    ...
    const maps = filtered.map((entry) => buildWpxHourBucketMap(entry.qsos));
    const order = buildHourKeyOrderFromMapsList(maps, filtered.map((entry) => entry.qsos));
    if (!order.length) {
      const message = range ? 'No WPX QSOs inside the locked time range.' : 'No WPX QSOs to analyze.';
      const htmlBlocks = slots.map((entry) => (entry.ready ? `<p>${message}</p>` : `<p>No ${entry.label} loaded.</p>`));
      return renderComparePanels(slots, htmlBlocks, 'wpx_by_hour_sheet');
    }
    const mode = normalizeWpxColumnMode(state.wpxColumnMode);
    const columnOrder = buildWpxColumnOrderFromMaps(maps, mode);
    if (!columnOrder.length) {
      const htmlBlocks = slots.map((entry) => (entry.ready ? '<p>No WPX columns to analyze.</p>' : `<p>No ${entry.label} loaded.</p>`));
      return renderComparePanels(slots, htmlBlocks, 'wpx_by_hour_sheet');
    }
    const htmlBlocks = slots.map((entry, idx) => (
  ...
  function computePeakPointsWindowFromQsos(qsos, pointsByQso, windowMinutes) {
    const windowMs = windowMinutes * 60000;
    const data = (qsos || [])
      .map((q) => {
        const qsoNumber = q?.qsoNumber;
        const pointsRaw = Number.isFinite(pointsByQso.get(qsoNumber))
          ? pointsByQso.get(qsoNumber)
          : Number(q?.points);
        return {
          ts: q?.ts,
  ...
  function buildRatesOperatorSummary(derived, qsos) {
    if (!derived?.hasPerQsoOperator) return [];
    const opBuckets = new Map();
    const windows = [10, 20, 30, 60, 120];
    const qsoList = Array.isArray(qsos) ? qsos : [];
    if (!qsoList.length) return [];
    qsoList.forEach((q) => {
      if (!Number.isFinite(q?.ts) || q?.isDupe) return;
      const op = String(q?.op || '').trim().toUpperCase();
      if (!op) return;
      if (!opBuckets.has(op)) opBuckets.set(op, []);
      opBuckets.get(op).push(q);
    });
    if (!opBuckets.size) return [];
    const pointsByIndex = getEffectivePointsByIndex(derived, qsoList);
    const pointsByQso = new Map();
    qsoList.forEach((q, idx) => {
      if (!q?.qsoNumber) return;
      const rawPoints = Number.isFinite(pointsByIndex[idx]) ? pointsByIndex[idx] : Number(q.points);
      pointsByQso.set(q.qsoNumber, Number.isFinite(rawPoints) ? rawPoints : 0);
    });

    const summaries = Array.from(opBuckets.entries()).map(([op, opQsos]) => {
      let bestQso = { count: 0, perMin: 0, perHour: 0, window: 0, startTs: null, endTs: null };
      for (const window of windows) {
        const peak = computePeakWindow(opQsos, window);
        const perMin = peak.count ? peak.count / window : 0;
        const perHour = peak.count ? (peak.count * 60) / window : 0;
        if (perHour > bestQso.perHour || (perHour === bestQso.perHour && peak.count > bestQso.count)) {
          bestQso = {
            count: peak.count,
            perMin,
            perHour,
            ...
            endTs: peak.endTs,
            startQso: peak.startQso,
            endQso: peak.endQso
          };
        }
      }
      let bestPoints = { points: null, perMin: null, perHour: null, window: null, startTs: null, endTs: null };
      for (const window of windows) {
        const peak = computePeakPointsWindowFromQsos(opQsos, pointsByQso, window);
        const perMin = peak.points ? peak.points / window : 0;
        const perHour = peak.points ? (peak.points * 60) / window : 0;
        if (bestPoints.points === null || perHour > bestPoints.perHour) {
          bestPoints = {
            points: peak.points,
            perMin,
            perHour,
    ...
    return summaries.sort((a, b) => a.op.localeCompare(b.op));
  }

  function renderRatesOperatorSummary(derived, qsos) {
    const items = buildRatesOperatorSummary(derived, qsos);
    if (!items.length) return '';
    const rows = items.map((item, idx) => {
      const qsoWindow = Number.isFinite(item.bestQso?.window) ? `${item.bestQso.window}` : 'N/A';
      const qsoCount = Number.isFinite(item.bestQso?.count) ? formatNumberSh6(item.bestQso.count) : 'N/A';
      const qsoPerMin = Number.isFinite(item.bestQso?.perMin) ? item.bestQso.perMin.toFixed(1) : 'N/A';
      const qsoPerHour = Number.isFinite(item.bestQso?.perHour) ? formatNumberSh6(Math.round(item.bestQso.perHour)) : 'N/A';
      const bestPoints = item.bestPoints || {};
      const pointsWindow = Number.isFinite(bestPoints.window) ? `${bestPoints.window}` : 'N/A';
      const points = Number.isFinite(bestPoints.points) ? formatPointValue(bestPoints.points) : 'N/A';
      const pointsPerMin = Number.isFinite(bestPoints.perMin) ? bestPoints.perMin.toFixed(1) : 'N/A';
      const pointsPerHour = Number.isFinite(bestPoints.perHour) ? bestPoints.perHour.toFixed(1) : 'N/A';
      const cls = idx % 2 === 0 ? 'td1' : 'td0';
      return `
        <div class="rates-row ${cls}">
          <div class="rates-cell"><b>${escapeHtml(item.op)}</b></div>
          <div class="rates-cell">${qsoWindow}</div>
          <div class="rates-cell">${qsoCount}</div>
          <div class="rates-cell">${qsoPerMin}</div>
          <div class="rates-cell">${qsoPerHour}</div>
          <div class="rates-cell">${pointsWindow}</div>
          <div class="rates-cell">${points}</div>
          <div class="rates-cell">${pointsPerMin}</div>
          <div class="rates-cell">${pointsPerHour}</div>
        </div>
      `;
    }).join('');
    return `
  ...
  function renderRates() {
    ...
    const qsos = state.qsoData?.qsos || [];
    const operatorRatesSummary = renderRatesOperatorSummary(state.derived, qsos);
    return `
      <div class="gradient">&nbsp;QSO rates</div>
      ${renderQsoRatesForData(qsos)}
      ${operatorRatesSummary}
  ...
  function buildMinuteMapFromList(minutes) {
    const minutesMap = new Map();
    if (!Array.isArray(minutes)) return minutesMap;
    minutes.forEach((minute) => {
      const value = Number(minute);
      if (Number.isFinite(value)) minutesMap.set(value, 1);
    });
    return minutesMap;
  }
  ...
  function renderPointsRatesCompareAligned() {
    ...
    const htmlBlocks = slots.map((entry) => (
      entry.ready
        ? `${renderRatesOperatorSummary(entry.snapshot.derived, entry.snapshot.qsoData?.qsos || [])}${renderPointsRatesForData(entry.snapshot.derived, entry.snapshot.qsoData?.qsos || [])}`
        : `<p>No ${entry.label} loaded.</p>`
    ));
  ...
  function renderOperatingStyleReportForDerived(derived, qsos = []) {
    ...
    const heuristicText = [
      ...
      `cluster time ≤${formatNumberSh6(meta.cabrilloRunWindowMaxMinutes || 0)} min`,
      `active RUN gap ≤${formatNumberSh6(meta.activeRunGapQsos || 0)} QSOs`,
      `gap time ≤${formatNumberSh6(meta.activeRunGapMaxMinutes || 0)} min`,
      `RUN seed halo ≤${formatNumberSh6(meta.activeRunSeedHaloMinutes || 0)} min`,
    ...
    const anchorText = meta.spotAnchoringUsed
      ? `Loaded Spots/RBN added ${formatNumberSh6(meta.spotAnchorCount || 0)} RUN anchors (Spots ${formatNumberSh6(anchorCounts.spots || 0)}, RBN ${formatNumberSh6(anchorCounts.rbn || 0)}, RBN bridge ${formatNumberSh6(anchorCounts.rbnMicro || 0)}).`
...
<kml xmlns="http://www.opengis.net/kml/2.2">
  ...
  function buildPassedQsoPairs(qsos, windowMinutes) {
    ...
    byCall.forEach((list) => {
      ...
      list.sort((a, b) => a.ts - b.ts || a.idx - b.idx);
      for (let i = 0; i < list.length - 1; i += 1) {
        let best = null;
        for (let j = i + 1; j < list.length; j += 1) {
          const delayMs = list[j].ts - list[i].ts;
          if (delayMs > windowMs) break;
          if (list[i].band === list[j].band) continue;
          best = { from: list[i], to: list[j], delayMs };
          break;
        }
        if (!best) continue;
        const key = `${best.from.idx}:${best.to.idx}`;
        if (seen.has(key)) continue;
        seen.add(key);
        pairs.push(best);
      }
    });
    pairs.sort((a, b) => a.from.ts - b.from.ts || a.to.ts - b.to.ts || a.from.idx - b.from.idx);
    return pairs;
  }

  function buildPassedTransitionStats(pairs) {
    const stats = new Map();
    (pairs || []).forEach((pair) => {
      const fromBand = (pair?.from?.band || '').trim().toUpperCase();
      const toBand = (pair?.to?.band || '').trim().toUpperCase();
      ...
      const delayMs = Math.max(0, Number(pair.delayMs) || 0);
      existing.count += 1;
      existing.totalDelayMs += delayMs;
      existing.minDelayMs = Math.min(existing.minDelayMs, delayMs);
      existing.maxDelayMs = Math.max(existing.maxDelayMs, delayMs);
      stats.set(key, existing);
    });
    return Array.from(stats.values()).sort((a, b) => (
      b.count - a.count
      || a.fromBand.localeCompare(b.fromBand)
      || a.toBand.localeCompare(b.toBand)
    ));
  }

  function formatPassedDelay(delayMs) {
    const totalSeconds = Math.max(0, Math.round((Number(delayMs) || 0) / 1000));
  ...
  function renderPassedTransitionMatrix(pairs) {
    const stats = buildPassedTransitionStats(pairs);
    if (!stats.length) return '';
    const rows = stats.map((entry, idx) => {
      const avgDelay = entry.count ? entry.totalDelayMs / entry.count : 0;
      const fromLabel = escapeHtml(formatBandLabel(entry.fromBand));
      const toLabel = escapeHtml(formatBandLabel(entry.toBand));
      return `
      <tr class="${idx % 2 === 0 ? 'td1' : 'td0'}">
        <td class="${bandClass(entry.fromBand)}">${fromLabel}</td>
        <td class="${bandClass(entry.toBand)}">${toLabel}</td>
        <td>${formatNumberHtmlSh6(entry.count)}</td>
        <td>${escapeHtml(formatPassedDelay(avgDelay))}</td>
        <td>${escapeHtml(formatPassedDelay(entry.minDelayMs))}</td>
        <td>${escapeHtml(formatPassedDelay(entry.maxDelayMs))}</td>
      </tr>
      `;
    }).join('');
    return `
      <h3>Band transitions</h3>
      <table class="mtc" style="margin-top:5px;margin-bottom:10px;text-align:right;">
        <tr class="thc"><th>From</th><th>To</th><th>Pairs</th><th>Avg delay</th><th>Fastest</th><th>Slowest</th></tr>
        ${rows}
      </table>
    `;
  }
  ...
  function renderPassedQsosForList(qsos, options = {}) {
    ...
    const summary = `
      <p>
        Passed QSO pairs: ${formatNumberSh6(pairs.length)}
        &nbsp;|&nbsp; Callsigns: ${formatNumberSh6(calls.size)}
        &nbsp;|&nbsp; Window: ${formatNumberSh6(windowMinutes)} minutes
      </p>
    `;
    const transitionMatrix = renderPassedTransitionMatrix(pairs);
    const rows = pairs.map((pair, idx) => {
      const from = pair.from.q || {};
      const to = pair.to.q || {};
      const fromNumber = from.qsoNumber || '';
      const toNumber = to.qsoNumber || '';
      const fromNumberAttr = escapeAttr(fromNumber);
      const toNumberAttr = escapeAttr(toNumber);
      const fromNumberCell = fromNumber ? `<a href="#" class="log-range" data-start="${fromNumberAttr}" data-end="${fromNumberAttr}">${formatNumberHtmlSh6(fromNumber)}</a>` : '';
      const toNumberCell = toNumber ? `<a href="#" class="log-range" data-start="${toNumberAttr}" data-end="${toNumberAttr}">${formatNumberHtmlSh6(toNumber)}</a>` : '';
      const call = escapeHtml(pair.from.call || '');
      const callAttr = escapeAttr(pair.from.call || '');
      const fromTime = escapeHtml(from.ts ? formatDateSh6(from.ts) : (from.time || ''));
      const toTime = escapeHtml(to.ts ? formatDateSh6(to.ts) : (to.time || ''));
      const fromBand = escapeHtml(formatBandLabel(from.band || ''));
      const toBand = escapeHtml(formatBandLabel(to.band || ''));
      const fromMode = escapeHtml(from.mode || '');
      const toMode = escapeHtml(to.mode || '');
      const delay = escapeHtml(formatPassedDelay(pair.delayMs));
      return `
      <tr class="${idx % 2 === 0 ? 'td1' : 'td0'}">
        <td><a href="#" class="log-call" data-call="${callAttr}">${call}</a></td>
        <td>${fromNumberCell}</td>
        <td>${fromTime}</td>
        <td class="${bandClass(from.band)}">${fromBand}</td>
        <td class="${modeClass(from.mode)}">${fromMode}</td>
        <td>${toNumberCell}</td>
        <td>${toTime}</td>
  ...
  function bindReportInteractions(reportId) {
    ...
    if (dom.viewContainer instanceof HTMLElement) {
      if (typeof viewContainerLogDrilldownHandler === 'function') {
        dom.viewContainer.removeEventListener('click', viewContainerLogDrilldownHandler);
      }
      viewContainerLogDrilldownHandler = (evt) => {
        ...
        if (target.classList.contains('log-range')) {
          ...
          const excludeDupes = target.dataset.excludeDupes === '1';
          if (!Number.isFinite(start) || !Number.isFinite(end)) return;
          state.logRange = { start, end, excludeDupes };
          state.logSearch = '';
          state.logFieldFilter = '';
        ...
        if (target.classList.contains('log-call')) {
          evt.preventDefault();
          const call = (target.dataset.call || '').trim().toUpperCase();
          if (!call) return;
          state.logRange = null;
      ...
      dom.viewContainer.addEventListener('click', viewContainerLogDrilldownHandler);
  ...
  function renderBreaksForDerived(derived, slotLabel, options = {}) {
    ...
    let operatorHtml = '';
    if (showOperatorBreaks) {
      const operatorRows = operatorSummary.map((item) => {
        const breakTime = `${formatMinutes(item.totalBreakMin)}`;
        const onAir = item.onAirMin != null ? formatMinutes(item.onAirMin) : 'N/A';
        const participation = item.participationMin != null ? formatMinutes(item.participationMin) : 'N/A';
        return `<tr><td>${escapeHtml(item.op)}</td><td>${participation}</td><td>${breakTime}</td><td>${item.breakCount}</td><td>${onAir}</td></tr>`;
      }).join('');
      operatorHtml = `
        <h3>Per-operator break time</h3>
        <table class="mtc" style="margin-top:5px;margin-bottom:10px;text-align:right;">
          <tr class="thc"><th>Operator</th><th>Participation</th><th>Break time</th><th>Break count</th><th>ON AIR</th></tr>
          ${operatorRows}
        </table>
      `;
    }
    if (!breakSummary.breaks.length) {
      return `
        ${slider}
        ${summaryHtml}
        ${operatorHtml}

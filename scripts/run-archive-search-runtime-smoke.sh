...
PORT="${ARCHIVE_SEARCH_RUNTIME_SMOKE_PORT:-8793}"
URL="http://127.0.0.1:${PORT}/tests/archive-search-runtime-smoke.html"
SESSION="sh6-archive-search-runtime-smoke-$RANDOM-$RANDOM"
SERVER_LOG="${ARCHIVE_SEARCH_RUNTIME_SMOKE_SERVER_LOG:-/tmp/sh6-archive-search-runtime-smoke-server.log}"
...
echo "[archive-search-runtime-smoke] Starting static server at ${URL}"
python3 -m http.server "${PORT}" --bind 127.0.0.1 --directory "${ROOT_DIR}" >"${SERVER_LOG}" 2>&1 &
HTTP_PID="$!"
sleep 1

if ! kill -0 "${HTTP_PID}" >/dev/null 2>&1; then
  echo "[archive-search-runtime-smoke] Failed to start local server. See ${SERVER_LOG}" >&2
  exit 1
fi
...
echo "[archive-search-runtime-smoke] status=${status}"
echo "${result_json}"

if [[ "${status}" != "PASS" ]]; then
  echo "[archive-search-runtime-smoke] Browser smoke test failed." >&2
  exit 1
fi

echo "[archive-search-runtime-smoke] PASS"

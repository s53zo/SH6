...
PORT="${COMPARE_INSIGHTS_VIEW_SMOKE_PORT:-8822}"
URL="http://127.0.0.1:${PORT}/tests/compare-insights-view-smoke.html"
SESSION="sh6-compare-insights-view-smoke-$RANDOM-$RANDOM"
SERVER_LOG="${COMPARE_INSIGHTS_VIEW_SMOKE_SERVER_LOG:-/tmp/sh6-compare-insights-view-smoke-server.log}"
...
echo "[compare-insights-view-smoke] Starting static server at ${URL}"
...
"${AGENT_BROWSER_BIN}" --session "${SESSION}" wait 250 >/dev/null

for width in 375 768 1024 1440; do
  "${AGENT_BROWSER_BIN}" --session "${SESSION}" set viewport "${width}" 900 >/dev/null
  "${AGENT_BROWSER_BIN}" --session "${SESSION}" wait 100 >/dev/null
  viewport_result="$("${AGENT_BROWSER_BIN}" --session "${SESSION}" eval "window.recordCompareInsightsViewport(${width})" 2>/dev/null | tr -d '\r')"
  echo "[compare-insights-view-smoke] viewport=${width} result=${viewport_result}"
done

"${AGENT_BROWSER_BIN}" --session "${SESSION}" eval "window.finishCompareInsightsViewSmoke()" >/dev/null
...
echo "[compare-insights-view-smoke] status=${status}"
echo "${result_json}"
[[ "${status}" == "PASS" ]] || { echo "[compare-insights-view-smoke] FAIL" >&2; exit 1; }
echo "[compare-insights-view-smoke] PASS"

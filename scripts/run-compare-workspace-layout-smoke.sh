...
PORT="${COMPARE_WORKSPACE_LAYOUT_SMOKE_PORT:-8804}"
URL="http://127.0.0.1:${PORT}/tests/compare-workspace-layout-smoke.html"
SESSION="sh6-compare-workspace-layout-smoke-$RANDOM-$RANDOM"
SERVER_LOG="${COMPARE_WORKSPACE_LAYOUT_SMOKE_SERVER_LOG:-/tmp/sh6-compare-workspace-layout-smoke-server.log}"
...
echo "[compare-workspace-layout-smoke] Starting static server at ${URL}"
python3 -m http.server "${PORT}" --bind 127.0.0.1 --directory "${ROOT_DIR}" >"${SERVER_LOG}" 2>&1 &
HTTP_PID="$!"
sleep 1

if ! kill -0 "${HTTP_PID}" >/dev/null 2>&1; then
  echo "[compare-workspace-layout-smoke] Failed to start local server. See ${SERVER_LOG}" >&2
  exit 1
fi
...
echo "[compare-workspace-layout-smoke] status=${status}"
echo "${result_json}"

if [[ "${status}" != "PASS" ]]; then
  echo "[compare-workspace-layout-smoke] Browser smoke test failed." >&2
  exit 1
fi

echo "[compare-workspace-layout-smoke] PASS"

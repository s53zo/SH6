...
PORT="${CQ_API_SMOKE_PORT:-8779}"
URL="http://127.0.0.1:${PORT}/tests/cq-api-browser-smoke.html"
SESSION="sh6-cqapi-smoke-$RANDOM-$RANDOM"
SERVER_LOG="${CQ_API_SMOKE_SERVER_LOG:-/tmp/sh6-cq-api-smoke-server.log}"
...
echo "[cq-api-smoke] status=${status}"
echo "${result_json}"

if [[ "${status}" != "PASS" ]]; then
  echo "[cq-api-smoke] Browser smoke test failed." >&2
  exit 1
fi

echo "[cq-api-smoke] PASS"

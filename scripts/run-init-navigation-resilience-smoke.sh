...
PORT="${INIT_NAVIGATION_RESILIENCE_SMOKE_PORT:-8825}"
URL="http://127.0.0.1:${PORT}/tests/init-navigation-resilience-smoke.html"
SESSION="sh6-init-navigation-resilience-smoke-$RANDOM-$RANDOM"
SERVER_LOG="${INIT_NAVIGATION_RESILIENCE_SMOKE_SERVER_LOG:-/tmp/sh6-init-navigation-resilience-smoke-server.log}"
...
echo "[init-navigation-resilience-smoke] Starting static server at ${URL}"
python3 -m http.server "${PORT}" --bind 127.0.0.1 --directory "${ROOT_DIR}" >"${SERVER_LOG}" 2>&1 &
HTTP_PID="$!"
sleep 1

if ! kill -0 "${HTTP_PID}" >/dev/null 2>&1; then
  echo "[init-navigation-resilience-smoke] Failed to start local server. See ${SERVER_LOG}" >&2
  exit 1
fi
...
echo "[init-navigation-resilience-smoke] status=${status}"
echo "${result_json}"

if [[ "${status}" != "PASS" ]]; then
  echo "[init-navigation-resilience-smoke] Browser smoke test failed." >&2
  exit 1
fi

echo "[init-navigation-resilience-smoke] PASS"

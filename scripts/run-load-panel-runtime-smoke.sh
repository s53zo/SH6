...
PORT="${LOAD_PANEL_RUNTIME_SMOKE_PORT:-8794}"
URL="http://127.0.0.1:${PORT}/tests/load-panel-runtime-smoke.html"
SESSION="sh6-load-panel-runtime-smoke-$RANDOM-$RANDOM"
SERVER_LOG="${LOAD_PANEL_RUNTIME_SMOKE_SERVER_LOG:-/tmp/sh6-load-panel-runtime-smoke-server.log}"
...
echo "[load-panel-runtime-smoke] Starting static server at ${URL}"
python3 -m http.server "${PORT}" --bind 127.0.0.1 --directory "${ROOT_DIR}" >"${SERVER_LOG}" 2>&1 &
HTTP_PID="$!"
sleep 1

if ! kill -0 "${HTTP_PID}" >/dev/null 2>&1; then
  echo "[load-panel-runtime-smoke] Failed to start local server. See ${SERVER_LOG}" >&2
  exit 1
fi
...
echo "[load-panel-runtime-smoke] status=${status}"
echo "${result_json}"

if [[ "${status}" != "PASS" ]]; then
  echo "[load-panel-runtime-smoke] Browser smoke test failed." >&2
  exit 1
fi

echo "[load-panel-runtime-smoke] PASS"

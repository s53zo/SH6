...
PORT="${MENU_VISIBILITY_SMOKE_PORT:-8805}"
URL="http://127.0.0.1:${PORT}/tests/menu-visibility-smoke.html"
SESSION="sh6-menu-visibility-smoke-$RANDOM-$RANDOM"
SERVER_LOG="${MENU_VISIBILITY_SMOKE_SERVER_LOG:-/tmp/sh6-menu-visibility-smoke-server.log}"
...
echo "[menu-visibility-smoke] Starting static server at ${URL}"
python3 -m http.server "${PORT}" --bind 127.0.0.1 --directory "${ROOT_DIR}" >"${SERVER_LOG}" 2>&1 &
HTTP_PID="$!"
sleep 1

if ! kill -0 "${HTTP_PID}" >/dev/null 2>&1; then
  echo "[menu-visibility-smoke] Failed to start local server. See ${SERVER_LOG}" >&2
  exit 1
fi
...
echo "[menu-visibility-smoke] status=${status}"
echo "${result_json}"

if [[ "${status}" != "PASS" ]]; then
  echo "[menu-visibility-smoke] Browser smoke test failed." >&2
  exit 1
fi

echo "[menu-visibility-smoke] PASS"

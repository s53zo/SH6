...
PORT="${NAVIGATION_RUNTIME_SMOKE_PORT:-8789}"
URL="http://127.0.0.1:${PORT}/tests/navigation-runtime-smoke.html"
SESSION="sh6-navigation-runtime-smoke-$RANDOM-$RANDOM"
SERVER_LOG="${NAVIGATION_RUNTIME_SMOKE_SERVER_LOG:-/tmp/sh6-navigation-runtime-smoke-server.log}"
...
echo "[navigation-runtime-smoke] Starting static server at ${URL}"
python3 -m http.server "${PORT}" --bind 127.0.0.1 --directory "${ROOT_DIR}" >"${SERVER_LOG}" 2>&1 &
HTTP_PID="$!"
sleep 1

if ! kill -0 "${HTTP_PID}" >/dev/null 2>&1; then
  echo "[navigation-runtime-smoke] Failed to start local server. See ${SERVER_LOG}" >&2
  exit 1
fi
...
echo "[navigation-runtime-smoke] status=${status}"
echo "${result_json}"

if [[ "${status}" != "PASS" ]]; then
  echo "[navigation-runtime-smoke] Browser smoke test failed." >&2
  exit 1
fi

echo "[navigation-runtime-smoke] PASS"

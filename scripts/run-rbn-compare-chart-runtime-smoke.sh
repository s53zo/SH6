...
PORT="${RBN_COMPARE_CHART_RUNTIME_SMOKE_PORT:-8813}"
URL="http://127.0.0.1:${PORT}/tests/rbn-compare-chart-runtime-smoke.html"
SESSION="sh6-rbn-compare-chart-runtime-smoke-$RANDOM-$RANDOM"
SERVER_LOG="${RBN_COMPARE_CHART_RUNTIME_SMOKE_SERVER_LOG:-/tmp/sh6-rbn-compare-chart-runtime-smoke-server.log}"
...
echo "[rbn-compare-chart-runtime-smoke] Starting static server at ${URL}"
python3 -m http.server "${PORT}" --bind 127.0.0.1 --directory "${ROOT_DIR}" >"${SERVER_LOG}" 2>&1 &
HTTP_PID="$!"
sleep 1

if ! kill -0 "${HTTP_PID}" >/dev/null 2>&1; then
  echo "[rbn-compare-chart-runtime-smoke] Failed to start local server. See ${SERVER_LOG}" >&2
  exit 1
fi
...
echo "[rbn-compare-chart-runtime-smoke] status=${status}"
echo "${result_json}"

if [[ "${status}" != "PASS" ]]; then
  echo "[rbn-compare-chart-runtime-smoke] Browser smoke test failed." >&2
  exit 1
fi

echo "[rbn-compare-chart-runtime-smoke] PASS"

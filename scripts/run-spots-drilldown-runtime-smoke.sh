...
PORT="${SPOTS_DRILLDOWN_RUNTIME_SMOKE_PORT:-8817}"
URL="http://127.0.0.1:${PORT}/tests/spots-drilldown-runtime-smoke.html"
SESSION="sh6-spots-drilldown-runtime-smoke-$RANDOM-$RANDOM"
SERVER_LOG="${SPOTS_DRILLDOWN_RUNTIME_SMOKE_SERVER_LOG:-/tmp/sh6-spots-drilldown-runtime-smoke-server.log}"
...
echo "[spots-drilldown-runtime-smoke] Starting static server at ${URL}"
python3 -m http.server "${PORT}" --bind 127.0.0.1 --directory "${ROOT_DIR}" >"${SERVER_LOG}" 2>&1 &
HTTP_PID="$!"
sleep 1

if ! kill -0 "${HTTP_PID}" >/dev/null 2>&1; then
  echo "[spots-drilldown-runtime-smoke] Failed to start local server. See ${SERVER_LOG}" >&2
  exit 1
fi
...
echo "[spots-drilldown-runtime-smoke] status=${status}"
echo "${result_json}"

if [[ "${status}" != "PASS" ]]; then
  echo "[spots-drilldown-runtime-smoke] Browser smoke test failed." >&2
  exit 1
fi

echo "[spots-drilldown-runtime-smoke] PASS"
